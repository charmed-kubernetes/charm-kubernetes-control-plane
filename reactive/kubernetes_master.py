...
def ceph_storage():
    ...
    # This is needed as we must allow privileged.
    reconfigure_apiserver(
        endpoint_from_flag('etcd.available'),
        force_privilage=True
    )
...
def is_privileged():
    ...
    if privileged == 'auto':
        return \
            ...
            is_state('kubernetes-master.ceph.configured')
    else:
...
@when('etcd.available')
def reconfigure_apiserver(etcd, force_privilage=False):
    configure_apiserver(
        etcd.get_connection_string(), force_privilage)
...
def configure_apiserver(etcd_connection_string, force_privilage=False):
    api_opts = {}
    ...
    if is_privileged() or force_privilage:
