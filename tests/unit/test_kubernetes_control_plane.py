...
def test_default_cni_changed():
    set_flag(CHARM_PREFIX + ".components.started")
    kubernetes_control_plane.default_cni_changed()
    assert not is_flag_set(CHARM_PREFIX + ".components.started")
...
def configure_apiserver(service_cidr_from_db, service_cidr_from_config):
    ...
    db = unitdata.kv()
    db.set(CHARM_PREFIX + ".service-cidr", service_cidr_from_db)
...
def test_service_cidr_greenfield_deploy():
    ...
    assert not is_flag_set(CHARM_PREFIX + ".had-service-cidr-expanded")
...
def test_status_set_on_incomplete_lb():
    ...
    set_flag(CHARM_PREFIX + ".system-monitoring-rbac-role.applied")
...
def test_status_set_on_failed_master_services(call, heal_handler, msd):
    ...
    test_heal_handler = {
        "kube-apiserver": {
            ...
            "clear_flags": [CHARM_PREFIX + ".apiserver.configured"],
        }
    }
    ...
    clear_flag.assert_called_with(CHARM_PREFIX + ".apiserver.configured")
    set_flag.assert_called_with(CHARM_PREFIX + ".components.failed")
...
def test_status_set_on_healed_master_services(call, heal_handler, msd):
    ...
    clear_flag.assert_called_with(CHARM_PREFIX + ".components.failed")
...
def test_create_token_sign_auth_requests(get_token, setup_tokens):
    ...
    assert not is_flag_set(CHARM_PREFIX + ".auth-webhook-tokens.setup")
    ...
    assert kube_control.sign_auth_request.call_count == 0
    assert is_flag_set(CHARM_PREFIX + ".auth-webhook-tokens.setup")
...
@mock.patch.object(kubernetes_control_plane, "render", autospec=True)
@mock.patch.object(kubernetes_control_plane, "get_ingress_address", autospec=True)
